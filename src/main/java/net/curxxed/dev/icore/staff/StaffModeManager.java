package net.curxxed.dev.icore.staff;
...
import net.curxxed.dev.icore.commands.staff.VanishCommand;
...
public class StaffModeManager {
    ...
    public boolean hasStaffPermission(Player player) {
        return player.hasPermission("icore.staff") || player.hasPermission("icore.admin")
                || player.hasPermission("icore.manager") || player.isOp();
    }
    ...
    public void modifyPlayerInfoPacket(PacketEvent event) {
        ...
        // Post-1.19 logic (ClientboundPlayerInfoUpdatePacket)
            ...
            try {
                // Minecraft 1.19+ rewrites this as a record-based structure.
                ...
                for (Object entry : entries) {
                    ...
                    if (p != null && staffModePlayers.contains(p.getUniqueId())) {
                        // Reflectively change the profile name (must create a new GameProfile or modify raw entry)
